...
public class ADWindowPanel extends AbstractADWindowPanel
{
	...
	public void onExport() {
		...
		GridTabExcelExporter export = new GridTabExcelExporter(ctx,curTab);
		File outFile = null;
		
		try{
			outFile = File.createTempFile("Export"+"."+curTab.getTableName()+".", "xls");
			export.export(outFile, null);
			
			AMedia media = null;
			media = new AMedia("Export"+"."+curTab.getTableName()+".xls",null,
								"application/octet-stream",outFile,true);
			
			Filedownload.save(media,outFile.getName());
		}
		catch(Exception ex){
			ex.printStackTrace();
		}
