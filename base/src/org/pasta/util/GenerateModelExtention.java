 ...
 */
public class GenerateModelExtention
{
	
	/**	Logger			*/
	private static CLogger	log	= CLogger.getCLogger (GenerateModelExtention.class);
	...
	public static void main (String[] args)
	{
		...
		log.info("Generate Model Extension  $Revision: 1.42 $");
		log.info("----------------------------------");
		//	first parameter
		...
		try
		{
			...
			while (rs.next())
			{
				new ModelInterfaceGenerator(rs.getInt(1), directory, packageName,entityColumn);
				new ModelClassGenerator(rs.getInt(1), directory, packageName,entityColumn);
				count++;
			}
 		}
		catch (Exception e)
