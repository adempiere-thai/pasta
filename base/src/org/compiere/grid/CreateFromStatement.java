...
public class CreateFromStatement extends CreateFrom 
{
			...
			Object AmtFrom, Object AmtTo, Object DocType, Object TenderType, String AuthCode)
	{
		...
		String sql = "SELECT p.DateTrx,p.C_Payment_ID,p.DocumentNo, p.C_Currency_ID,c.ISO_Code, p.PayAmt,"
			...
			+ " bp.Name , pay.withholdingamt , pay.isreceipt "
		...
		try
		{
			...
			while (rs.next())
			{
				...
				BigDecimal withholdingTaxAmt = rs.getBigDecimal(9);
				if("N".equals(rs.getString(10)))
					withholdingTaxAmt = withholdingTaxAmt.negate();
				...
				line.add(rs.getBigDecimal(6).subtract(withholdingTaxAmt));      //  4-PayAmt
				line.add(rs.getBigDecimal(7).subtract(withholdingTaxAmt));      //  5-Conv Amt
